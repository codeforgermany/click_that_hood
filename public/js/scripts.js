    ...
    if (CITY_DATA[cityId].pointsInsteadOfPolygons) {
      ...
      for (var z in geoData.features[i].geometry.coordinates) {
        for (var j in geoData.features[i].geometry.coordinates[z]) {
          if (
            geoData.features[i].geometry.coordinates[z][j].length &&
...
/*
    ...
    var width =
      (globalScale / (D3_DEFAULT_SCALE * 360)) * lonSpread * MAPS_DEFAULT_SCALE;
    ...
    switch (mapHorizontalOffset) {
      ...
      case MAP_HORIZONTAL_OFFSET_REVERSED:
        projection = projection
          .center([centerLon - 180, centerLat])
          .rotate([180, 0]);
        break;
    }
    ...
    for (var i = 0, el1; (el1 = els[i]); i++) {
      ...
      for (var j = 0, el2; (el2 = els[j]); j++) {
        ...
        if (dist < radius) {
          ...
          el1.setAttribute(
            "transform",
            "translate(" + (newX + radius / 2) + "," + (newY + radius / 2) + ")"
          );
...
function removeSmallNeighborhoods() {
  ...
  for (var i = 0, el; (el = els[i]); i++) {
    ...
    if (
      boundingBox.width < smallNeighborhoodThreshold ||
  ...
  if (CITY_DATA[cityId].languages) {
    ...
    for (var name in CITY_DATA[cityId].languages) {
      if (
        ...
        window.localStorage["prefer-" + name + "-to-" + defaultLanguage] ===
          "yes"
      ) {
        language = name;
        return;
...
function updateLanguagesSelector() {
  ...
  var el = document.querySelector(
    'header .languages button[name="' + language + '"]'
  );
...
function createMainMenuMap() {
  ...
  if (currentGeoLat) {
    ...
    feature.geometry = {
      type: "Point",
      coordinates: [currentGeoLon, currentGeoLat],
    };
...
function addPaddedIslandNeighborhoods() {
  ...
  for (var i = 0, el; (el = els[i]); i++) {
    ...
    // TODO const
    var needsPadding = area < 100 && area / boundingBoxArea < 0.35;
...
function animateCorrectNeighborhoodGuess(el) {
  ...
  if (el.getAttribute("transformX")) {
    ...
    window.setTimeout(function () {
      ...
      animEl.style.MozTransform =
        "translate(" + x + "px, " + y + "px) scale(6)";
      animEl.style.webkitTransform =
        "translate(" + x + "px, " + y + "px) scale(6)";
    }, 50);
  } else {
    window.setTimeout(function () {
  ...
  for (var i = 0, el; (el = els[i]); i++) {
    ...
    if (el.getAttribute("composited")) {
      ...
      for (var j in names) {
        var name = names[j];
        if (
          neighborhoodsGuessed.indexOf(name) == -1 &&
          neighborhoodsToBeGuessed.indexOf(name) != -1
        ) {
          allGuessed = false;
        }
        if (
...
function onNeighborhoodClick(el) {
  ...
  if (guessed) {
    ...
    if (CITY_DATA[cityId].extraData) {
      var correctEl = getHighlightableNeighborhoodEl(
...
function updateGameProgress() {
  ...
  document
    .querySelector("#count-time-wrapper-wrapper")
...
function updateNeighborhoodDisplay() {
  if (neighborhoodToBeGuessedNext) {
    ...
    document
      .querySelector("#neighborhood-guess-wrapper")
      .classList.add("visible");
  } else {
    document
      .querySelector("#neighborhood-guess-wrapper")
...
function gameOverPart2() {
  ...
  document.querySelector("#number-identified").innerHTML = easyMode
    ? easyModeCount
    : "all";
...
function prepareMapBackground() {
  ...
  var size =
    (((globalScale * 0.0012238683395795992 * 0.995) / 2) * 0.8) / 2 / 4;
...
/*
    ...
    if((cityId === "africa") || CITY_DATA[cityId].pointsInsteadOfPolygons) {geoBounds = d3.geoBounds(geoBoundsObj); }
       ...
       // the browser uses the transform-translate bit, but ignores transformX and transformY 
        mapSvg
          ...
          .attr("transformX", function (d) {
            return projection(d.geometry.coordinates)[0] - radius / 2;
          })
          .attr("transformY", function (d) {
            return projection(d.geometry.coordinates)[1] - radius / 2;
          })
...
function getNeighborhoodNoun(plural) {
  if (!plural) {
    return (
      (CITY_DATA[cityId].neighborhoodNoun &&
        CITY_DATA[cityId].neighborhoodNoun[0]) ||
      DEFAULT_NEIGHBORHOOD_NOUN_SINGULAR
    );
  } else {
    return (
      (CITY_DATA[cityId].neighborhoodNoun &&
        CITY_DATA[cityId].neighborhoodNoun[1]) ||
      DEFAULT_NEIGHBORHOOD_NOUN_PLURAL
    );
  }
}
...
function preparePage() {
  ...
  if (
    !name ||
    name == COUNTRY_NAME_USA ||
    name == CITY_DATA[cityId].locationName ||
    (name == "U.K." && CITY_DATA[cityId].locationName == "United Kingdom")
  ) {
    name = "";
    document
      .querySelector("header .location-name")
      .classList.add("no-state-or-country");
  } else {
    document
      .querySelector("header .location-name")
      .classList.remove("no-state-or-country");
  }
...
function languageChange(event) {
  ...
  if (newLanguage != defaultLanguage) {
    window.localStorage["prefer-" + newLanguage + "-to-" + defaultLanguage] =
      "yes";
  }
...
function prepareLocationList() {
  ...
  ids.sort(function (a, b) {
    return (CITY_DATA[a].longLocationName || CITY_DATA[a].locationName) >
      (CITY_DATA[b].longLocationName || CITY_DATA[b].locationName)
      ? 1
      : -1;
  });
  ...
  var el = document.createElement("li");
  el.innerHTML =
    '<a target="_blank" class="add-your-city" href="' +
    ADD_YOUR_CITY_URL +
    '">Add your city…</a>';
  document.querySelector(".menu .locations").appendChild(el);
...
function testBrowser() {
  ...
  var mobile =
    window.matchMedia && window.matchMedia("(max-device-width: 568px)").matches;

  if (mobile) {
    document.getElementById("mobile").className += " visible";
    document
      .querySelector("#mobile button")
      .addEventListener("click", ignoreMobileBrowserWarning);
  }
